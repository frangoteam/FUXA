...
Dygraph.prototype.updateSelection_ = function(opt_animFraction) {
  ...
  if (this.selPoints_.length > 0) {
    ...
    ctx.save();
    if (this.getBooleanOption("drawSelectedPoint")) {
...
Dygraph.prototype.predraw_ = function() {
  ...
  //console.log("size: " + this.width_ + " x " + this.height_);
...
Dygraph.findPos = function(obj) {
  ...
  if (obj.offsetParent) {
    ...
    while (1) {
      ...
      // NOTE the if statement here is for canvas in SVG:foreignObject
      if (copyObj.nodeName === 'foreignObject') {
        //   if (!copyObj.parentNode) {
            //   break;
        //   }
        //   copyObj = copyObj.parentNode;
          var domRect = copyObj.getBoundingClientRect();
...
legend.generateLegendHTML = function (g, x, sel_points, oneEmWidth) {
  ...
  var html, sepLines, i, dash, strokePattern, y;
  ...
  for (i = 1; i < labels.length; i++) {
      ...
      for (y = 0; y < sel_points.length; y++) {
          if (sel_points[y].name == pt.name) {
              ...
              var yOptView = yOptViews[series.axis - 1];
              var fmtFunc = yOptView('valueFormatter');
              yval = fmtFunc(pt.yval, yOptView, pt.name, g);
              break;
          }
      }
      if (sepLines) html += "<br/>";
...
Dygraph.OPTIONS_REFERENCE =  // <JSON>
{
  ...
  "drawSelectedPoint": {
      ...
      "labels": ["Interactive Elements"],
      "type": "boolean",
      "description": "Draw the selected point when the mouse is over the chart area."
  }
}
;  // </JSON>
// NOTE: in addition to parsing as JS, this snippet is expected to be valid
