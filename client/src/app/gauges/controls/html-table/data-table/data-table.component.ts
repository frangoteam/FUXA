...
export class DataTableComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    private updateTableIfChanged(): boolean {
        ...
        this.updateDataSourceForCurrentPage();
    ...
    selectPageSize(value: number) {
        ...
        this.currentPageIndex = 0; // Reset to first page when changing page size
        this.updateDataSourceForCurrentPage();
        this.cdr.detectChanges();
        this.isPageSizeDropdownOpen = false;
    }

    private updateDataSourceForCurrentPage() {
    ...
    getPageInfo(): string {
        ...
        const totalPages = Math.ceil(totalItems / this.selectedPageSize);
        const startItem = this.currentPageIndex * this.selectedPageSize + 1;
    ...
    onDateRange() {
        let dialogRef = this.dialog.open(DaterangeDialogComponent, {
            ...
            data: {
                headerColor: this.tableOptions.header.color,
                headerBg: this.tableOptions.header.background,
                rowColor: this.tableOptions.row.color,
                rowBg: this.tableOptions.row.background,
                buttonBg: this.tableOptions.toolbar?.buttonColor,
                buttonColor: this.tableOptions.toolbar?.color || this.tableOptions.header.color
            }
        });
    ...
    private bindTableControls(): void {
        // Custom pagination - no MatPaginator needed
