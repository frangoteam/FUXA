...
export class ParameterTableComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
    ...
    private updatePagedData() {
        const startIndex = this.currentPageIndex * this.selectedPageSize;
    ...
    onSortChange(sort: Sort) {
        ...
        // Map data with original indices to maintain text rows in their places
        const indexedData = this.tableData.map((row, index) => ({ row, index }));
        
        const dataRows = indexedData.filter(item => item.row.type !== TableRowType.text);
        const textRows = indexedData.filter(item => item.row.type === TableRowType.text);

        // Sort only the data rows
        dataRows.sort((a, b) => {
            const isAsc = sort.direction === 'asc';
            const valA = a.row[sort.active]?.value ?? '';
            const valB = b.row[sort.active]?.value ?? '';
            ...
            return (String(valA).toLowerCase() < String(valB).toLowerCase() ? -1 : 1) * (isAsc ? 1 : -1);
        });

        // Reconstruct tableData: keep text rows at their original indices, fill others with sorted data
        const result = new Array(this.tableData.length);
        
        // Place text rows first
        textRows.forEach(item => {
            result[item.index] = item.row;
        });
        
        // Fill empty slots with sorted data rows
        let dataIdx = 0;
        for (let i = 0; i < result.length; i++) {
            if (result[i] === undefined) {
                result[i] = dataRows[dataIdx].row;
                dataIdx++;
            }
        }
    ...
    canGoNext(): boolean {
        const totalPages = Math.ceil(this.tableData.length / this.selectedPageSize);
    ...
    getPageInfo(): string {
        ...
        const startItem = totalItems > 0 ? (this.currentPageIndex * this.selectedPageSize + 1) : 0;
        const endItem = Math.min((this.currentPageIndex + 1) * this.selectedPageSize, totalItems);
