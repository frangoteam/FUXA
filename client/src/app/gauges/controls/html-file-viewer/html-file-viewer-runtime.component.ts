...
export class HtmlFileViewerRuntimeComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(private http: HttpClient, private snackBar: MatSnackBar, private authService: AuthService, private sanitizer: DomSanitizer, private translateService: TranslateService) {}
    ...
    loadFiles() {
        ...
        this.http.get(`/api/resources/browse?path=${encodeURIComponent(this.property.directory)}`)
            .subscribe({
                next: (response: any) => {
                ...
                error: (error) => {
                    ...
                    this.snackBar.open('Error loading files: ' + error.message, 'OK', { duration: 3000 });
                }
            });
    }
