...
@Component({
    selector: 'app-html-file-viewer-runtime',
    templateUrl: './html-file-viewer.component.html',
    styleUrls: ['./html-file-viewer.component.css']
})
export class HtmlFileViewerRuntimeComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    loadFiles() {
        ...
        console.log('Loading files from directory:', this.property.directory);

        this.resourcesService.browse(this.property.directory)
            .subscribe({
                next: (response: any) => {
                ...
                error: (error) => {
                    ...
                    this.toastNotifierService.notifyError('file-viewer.load-error', error.message);
                }
            });
    }
    ...
    deleteFile(file: FileItem) {
        ...
        title.textContent = this.translateService.instant('file-viewer.delete-title');
        title.style.cssText = `
            margin: 0;
            font-size: 18px;
            font-weight: 500;
        ...
        message.textContent = this.translateService.instant('file-viewer.delete-confirm', { filename: file.name });
        ...
        deleteBtn.onclick = () => {
            closeDialog();
            this.resourcesService.deleteResource(file.path)
                .subscribe({
                    next: () => {
                        this.toastNotifierService.notifySuccess('file-viewer.delete-success');
                        this.loadFiles();
                    },
                    error: (error) => {
                        this.toastNotifierService.notifyError('file-viewer.delete-error', error.message);
                    }
                });
        };
