            ...
            if (chart.lines) {
                for (var i = 0; i < chart.lines.length; i++) {
                    if (chart.lines[i].device === device.id) {
                        if (tags.map(function (x) { return x.id; }).indexOf(chart.lines[i].id) === -1) {
                            toremove.push(i);
                        }
                    }
                }
