 ...
 * @param {*} deviceid
 * @param {*} query
 * @param {*} userId - User ID for authorization logging
 * @param {*} userGroups - User groups for authorization validation
 */
function executeOdbcQuery(deviceid, query, userId, userGroups) {
    return new Promise(function (resolve, reject) {
        ...
        // Validate device has ODBC support
        if (!device.getComm || !device.getComm().executeQuery) {
            reject('Device does not support query execution');
            return;
        }
        
        // Execute query with parameterized support
