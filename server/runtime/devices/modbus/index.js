...
function MODBUSclient(_data, _logger, _events, _runtime) {
		...
		return new Promise(async function (resolve, reject) {
			...
			if (data.property.socketReuse) {
				...
				if (
						...
						(type === ModbusTypes.TCP ||
							(type === ModbusTypes.RTU &&
								data.property.baudrate &&
								data.property.databits &&
								data.property.stopbits &&
								data.property.parity))
					) {
					try {
						// Check if connection already exists in socketPool
						let connectionExists = false;
						if (data.property.socketReuse === ModbusReuseModeType.ReuseSerial && runtime.socketPool.has(data.property.address)) {
							// Use existing client connection
							client = runtime.socketPool.get(data.property.address);
							connectionExists = true;
						}
						
						if (!connectionExists && !client.isOpen && _checkWorking(true)) {
							logger.info(
							...
							// Connection already exists or client is open
							if (data.property.slaveid) {
								// set the client's unit id
								client.setID(
									parseInt(data.property.slaveid),
				...
				logger.error(`'${data.name}' connect error! ${err}`);
				_emitStatus("connect-error");
				_clearVarsValue();
				reject();
				if (!utils.isNullOrUndefined(socketRelease)) {
	...
	this.disconnect = function () {
		return new Promise(function (resolve, reject) {
			_checkWorking(false);
			// Don't close the connection if it's being reused
			if (data.property.socketReuse) {
	...
	this._polling = async function () {
		...
		if (_checkWorking(true)) {
			try {
				...
				// Set the client's unit id before polling
				if (data.property.slaveid) {
	...
	this.load = function (_data) {
		...
		for (var id in data.tags) {
			try {
				...
				if (!memory[memaddr].Items[offset]) {
					memory[memaddr].Items[offset] = new MemoryItem(
						data.tags[id].type,
						offset,
					);
				}
				...
				if (offset < memory[memaddr].Start) {
					if (memory[memaddr].Start != 65536) {
						...
						memory[memaddr].MaxSize =
							datatypes[data.tags[id].type].WordLen;
						memory[memaddr].Start = offset;
					}
				} else {
					var len =
			...
			.forEach(function (key) {
				try {
					...
					if (nextAdr < adr) {
						...
						mits.Start =
							lastStart - getMemoryAddress(lastStart, false);
						mits.MaxSize = lastAdrSize - lastStart;
						var token = Math.trunc(offset / TOKEN_LIMIT);
						lastMemAdr = getMemoryAddress(lastStart, true, token);
						mits.Items = getMemoryItems(
							memory[lastMemAdr].Items,
							mits.Start,
							mits.MaxSize,
						);
						...
						mixItemsMap[lastStart].MaxSize =
							lastAdrSize - lastStart;
						mixItemsMap[lastStart].Items = getMemoryItems(
							memory[lastMemAdr].Items,
							mixItemsMap[lastStart].Start,
							mixItemsMap[lastStart].MaxSize,
						);
					}
		...
		// Reset working flag to allow reconnection
		working = false;
	};
	...
	this.getTagProperty = function (id) {
		if (memItemsMap[id]) {
			return {
				id: id,
				name: id,
				type: memItemsMap[id].type,
				format: memItemsMap[id].format,
			};
		} else {
			return null;
		}
		...
		if (data.tags[sigid]) {
			...
			if (data.tags[sigid].scaleWriteFunction) {
				...
				if (data.tags[sigid].scaleWriteParams) {
					const extraParamsWithValues = JSON.parse(
						data.tags[sigid].scaleWriteParams,
					);
				...
				try {
					const bufVal = await runtime.scriptsMgr.runScript(
						script,
						false,
					);
					if (Array.isArray(bufVal)) {
						if (bufVal.length % 2 !== 0) {
							logger.error(
								`'${data.tags[sigid].name}' setValue script error, returned buffer invalid must be mod 2`,
							);
							return false;
						}
						val = [];
					...
					logger.error(
						`'${data.tags[sigid].name}' setValue script error! ${error.toString()}`,
					);
					return false;
				}
			} else {
			...
			logger.error(
				`'${data.name}' setValue(${sigid}, ${value}) Tag not found`,
				true,
				true,
			);
		}
	...
	var _connect = function (callback) {
		try {
			if (type === ModbusTypes.RTU) {
				...
				// Reuse RTU connection if available
				...
				if (
					...
					ModbusOptionType.RTUBufferedPort
				) {
					client.connectRTUBuffered(
						data.property.address,
						rtuOptions,
						function(err) {
							if (!err && data.property.socketReuse === ModbusReuseModeType.ReuseSerial) {
								// Store the client for reuse
								runtime.socketPool.set(data.property.address, client);
							}
							callback(err);
						}
					);
				...
				if (data.property.address.indexOf(":") !== -1) {
					addr = data.property.address.substring(
						0,
						data.property.address.indexOf(":"),
					);
					var temp = data.property.address.substring(
						data.property.address.indexOf(":") + 1,
					);
					port = parseInt(temp);
				}
				//reuse socket
	...
	var _readMemory = function (memoryAddress, start, size, vars) {
		return new Promise((resolve, reject) => {
			...
			if (memoryAddress === ModbusMemoryAddress.CoilStatus) {
					...
					(res) => {
						if (res.data) {
							vars.map((v) => {
								...
								let value = datatypes[v.type].parser(
									res.buffer,
									bitoffset,
									bit,
								);
				...
				// Digital Inputs (Read 100001-165536)
				...
				// Input Registers (Read  300001-365536)
								...
								try {
									...
									let buffer = Buffer.from(
										res.buffer.slice(
											byteoffset,
											byteoffset +
												datatypes[v.type].bytes,
										),
									);
									let value =
										datatypes[v.type].parser(buffer);
									v.changed = value !== v.rawValue;
									v.rawValue = value;
								} catch (err) {
									console.error(err);
				...
				// Holding Registers (Read/Write  400001-465535)
						...
						if (res.data) {
							vars.map((v) => {
								...
								let buffer = Buffer.from(
									res.buffer.slice(
										byteoffset,
										byteoffset + datatypes[v.type].bytes,
									),
								);
		...
		return new Promise((resolve, reject) => {
			if (memoryAddress === ModbusMemoryAddress.CoilStatus) {
				...
				if (value.length > 2 || data.property.forceFC16) {
					client.writeRegisters(start, value).then(
						(res) => {
							resolve();
						},
						(reason) => {
					...
					client.writeRegister(start, value).then(
						(res) => {
							resolve();
						},
						(reason) => {
		...
		if (someval) {
			...
			for (var id in tempTags) {
				if (!utils.isNullOrUndefined(tempTags[id].rawValue)) {
					tempTags[id].value = await deviceUtils.tagValueCompose(
						tempTags[id].rawValue,
					...
					if (
						this.addDaq &&
						deviceUtils.tagDaqToSave(tempTags[id], timestamp)
					) {
		...
		if (address < ModbusMemoryAddress.DigitalInputs) {
			...
			if (askey) {
				return formatAddress(
					ModbusMemoryAddress.HoldingRegisters,
					token,
				);
			}
	...
	const getMemoryItems = function (items, start, size) {
		...
		for (var itemidx in items) {
			if (
				items[itemidx].offset >= start &&
				items[itemidx].offset < start + size
			) {
...
const ModbusMemoryAddress = {
	CoilStatus: 0,
	DigitalInputs: 100000,
	InputRegisters: 300000,
	HoldingRegisters: 400000,
};
