        ...
        return new Promise(function (resolve, reject) {
            if (
                ...
                try {
                    ...
                    if (
                        !connectionManager.isConnected(connection.key) &&
                        _checkWorking(true)
                    ) {
                        logger.info(
                        ...
                        _connect(function (err) {
                            if (err) {
                                ...
                                logger.info(
                                    `'${data.name}' connected! (Slave ID: ${connection.slaveId})`,
                                    true,
                                );
                        ...
                        logger.info(
                            `'${data.name}' using shared connection (Slave ID: ${connection.slaveId})`,
                            true,
                        );
                        _emitStatus("connect-ok");
                        resolve();
    ...
    this.disconnect = function () {
        return new Promise(function (resolve, reject) {
            ...
            if (!client.isOpen) {
                ...
                logger.info(
                    `'${data.name}' disconnected (connection released)`,
                    true,
                );
                _emitStatus("connect-off");
                _clearVarsValue();
                ...
                if (!data.property.options) {
                    for (var memAddr in memory) {
                        try {
                            ...
                            if (readVars && readVars.length > 0)
                                results.push(readVars);
                            await delay(readDelay);
                        } catch (err) {
                            let errMsg =
                                err.message ||
                    ...
                    for (var mixAddr in mixItemsMap) {
                        try {
                            ...
                            if (mixReadVars && mixReadVars.length > 0)
                                results.push(mixReadVars);
                            await delay(readDelay);
                        } catch (err) {
                            let errMsg =
                                err.message ||
