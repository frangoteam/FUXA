...
function ODBCclient(_data, _logger, _events) {
    ...
    this.executeQuery = function (query, params) {
        var self = this;
        return new Promise(async function (resolve, reject) {
            // Add query to queue with optional parameters for parameterized queries
            queryQueue.push({ query: query, params: params || [], resolve: resolve, reject: reject });
    ...
    this._processQueryQueue = function () {
        ...
        // Execute the query with parameters (for parameterized query support)
        self._executeQueryInternal(query, params).then(function(result) {
            resolve(result);
            _checkWorking(false);
     ...
     * @param {string} query - SQL query string with optional ? placeholders for parameterized queries
     * @param {array} params - Optional array of parameters for parameterized queries (replaces ? placeholders)
     */
    this._executeQueryInternal = function (query, params) {
        var self = this;
        return new Promise(async function (resolve, reject) {
            try {
                if (self.connection) {
                    ...
                    // If parameterized query, use params array to prevent SQL injection
                    if (params && params.length > 0) {
                        try {
                            const result = await self.connection.query(normalizedQuery, params);
                            resolve(result);
                            return;
                        } catch (err) {
                            logger.error(`'${data.name}' parameterized query failed: ${err}`);
                            reject(err);
                            return;
                        }
                    }
                    ...
                    // Try single statement as-is first (only for non-parameterized queries)
                    try {
