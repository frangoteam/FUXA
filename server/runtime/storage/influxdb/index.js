...
function Influx(_settings, _log) {
    ...
    this.close = function () {
        try {
            ...
            writeApi.close().then(() => {
                console.log("influxdb-close FINISHED");
            })
            .catch((e) => {
                console.error(`influxdb-close failed! ${e}`);
            });
        } catch (error) {
            console.error(`influxdb-close failed! ${error}`);
        }
    }
    ...
    this.addDaqValues = function (tags) {
        for (var tagid in tags) {
            if (!tags[tagid].daq || !tags[tagid].daq.enabled) {
                continue;
            }
            writePoint(tags[tagid]);
        }
    ...
    this.getDaqValue = function (tagid, fromts, tots) {
        return new Promise(function (resolve, reject) {
            ...
            const query = flux`from(bucket: "${settings.daqstore.bucket}") |> range(start: -1d) |> filter(fn: (r) => r._measurement == "${tagid}")`;
    ...
    function writePoint(tag) {
        try {
            ...
            console.error(`influxdb-writePoints failed! ${error}`);
        }
    }
