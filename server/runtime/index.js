...
function init(_io, _api, _settings, _log, eventsMain) {
  ...
  var checkInit = function () {
    if (
      !events.listenerCount("init-plugins-ok") &&
    ...
    socket.on(Events.IoEventTypes.DEVICE_PROPERTY, (message) => {
      try {
        if (message && message.endpoint && message.type) {
          devices
            .getSupportedProperty(message.endpoint, message.type)
            .then((result) => {
    ...
    socket.on(Events.IoEventTypes.DEVICE_BROWSE, (message) => {
      try {
        if (message) {
          if (message.device) {
            devices
              .browseDevice(message.device, message.node, function (nodes) {
    ...
    socket.on(Events.IoEventTypes.DEVICE_NODE_ATTRIBUTE, (message) => {
      try {
        if (message) {
          if (message.device) {
            devices
              .readNodeAttribute(message.device, message.node)
              .then((result) => {
                io.emit(Events.IoEventTypes.DEVICE_NODE_ATTRIBUTE, message);
              })
              .catch(function (err) {
                logger.error(
                  `${Events.IoEventTypes.DEVICE_NODE_ATTRIBUTE}: ${err}`
                );
                message.error = err;
                io.emit(Events.IoEventTypes.DEVICE_NODE_ATTRIBUTE, message);
              });
      ...
      try {
        if (msg && msg.from && msg.to && msg.sids && msg.sids.length) {
          ...
          for (let i = 0; i < msg.sids.length; i++) {
            dbfncs.push(
              daqstorage.getNodeValues(msg.sids[i], msg.from, msg.to)
            );
          }
          Promise.all(dbfncs).then(
            (values) => {
            ...
            (reason) => {
              if (reason && reason.stack) {
                logger.error(
                  `${Events.IoEventTypes.DAQ_QUERY}: ${reason.stack}`
                );
              } else {
    ...
    socket.on(Events.IoEventTypes.DEVICE_WEBAPI_REQUEST, (message) => {
      try {
        if (message && message.property) {
          devices
            .getRequestResult(message.property)
            .then((result) => {
              message.result = result;
            ...
            .catch(function (err) {
              logger.error(
                `${Events.IoEventTypes.DEVICE_WEBAPI_REQUEST}: ${err}`
              );
              message.error = err;
              io.emit(Events.IoEventTypes.DEVICE_WEBAPI_REQUEST, message);
            });
          ...
          logger.error(
            `${Events.IoEventTypes.DEVICE_WEBAPI_REQUEST}: wrong message`
          );
          message.error = "wrong message";
          io.emit(Events.IoEventTypes.DEVICE_WEBAPI_REQUEST, message);
        }
    ...
    socket.on(Events.IoEventTypes.DEVICE_TAGS_REQUEST, (message) => {
      try {
        if (message && message.deviceId) {
          devices
            .getDeviceTagsResult(message.deviceId)
            .then((result) => {
              message.result = result;
            ...
            .catch(function (err) {
              logger.error(
                `${Events.IoEventTypes.DEVICE_TAGS_REQUEST}: ${err}`
              );
              message.error = err;
              io.emit(Events.IoEventTypes.DEVICE_TAGS_REQUEST, message);
            });
          ...
          logger.error(
            `${Events.IoEventTypes.DEVICE_TAGS_REQUEST}: wrong message`
          );
          message.error = "wrong message";
          io.emit(Events.IoEventTypes.DEVICE_TAGS_REQUEST, message);
        }
  ...
  return new Promise(function (resolve, reject) {
    try {
      if (cmd === project.ProjectDataCmdType.SetDevice) {
        ...
        cmd === project.ProjectDataCmdType.SetAlarm ||
        cmd === project.ProjectDataCmdType.DelAlarm
      ) {
        alarmsMgr.reset();
        notificatorMgr.reset();
      } else if (
        cmd === project.ProjectDataCmdType.SetNotification ||
        cmd === project.ProjectDataCmdType.DelNotification
      ) {
        notificatorMgr.reset();
        ...
        cmd === project.ProjectDataCmdType.SetReport ||
        cmd === project.ProjectDataCmdType.DelReport
      ) {
        jobsMgr.reset();
      }
      resolve(true);
