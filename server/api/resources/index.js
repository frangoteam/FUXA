            ...
            if (res.statusCode === 403) {
                ...
                try {
                    ...
                    if (dirPath && dirPath.startsWith('/_')) {
                        // Normalize backslashes (Windows) to forward slashes for prefix check
                        const normalizedPath = dirPath.split(path.sep).join(path.posix.sep);
                        
                        // Map server paths to actual filesystem paths
                        if (normalizedPath.startsWith('/_reports')) {
                            dirPath = path.join(runtime.settings.reportsDir, dirPath.substring('/_reports'.length));
                        } else if (normalizedPath.startsWith('/_images')) {
                            dirPath = path.join(runtime.settings.imagesFileDir, dirPath.substring('/_images'.length));
                        } else if (normalizedPath.startsWith('/_widgets')) {
