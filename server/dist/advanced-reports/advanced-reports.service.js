...
class AdvancedReportsApiService {
    ...
    syncDefaultTemplates() {
        try {
            ...
            const destDir = path.join(this.runtime.settings.workDir, '../_reports/templates');
    ...
    subscribeToTriggerTags() {
        try {
            ...
            let traditionalReports = [];
            if (fs.existsSync(reportsPath)) {
                traditionalReports = JSON.parse(fs.readFileSync(reportsPath, 'utf8'));
            }
            // Load advanced reports
            const configsDir = path.join(this.runtime.settings.workDir, '../_reports/configs');
            let advancedReports = [];
            ...
            for (const report of allReports) {
                ...
                if (report.triggerTags && Array.isArray(report.triggerTags)) {
                    report.triggerTags.forEach((tag) => triggerTags.add(tag));
                }
    ...
    app() {
         ...
         */
        advancedReportsApp.post('/api/advanced-reports/test-generate', this.secureFnc, async (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
                ...
                try {
                    ...
                    const configPath = path.join(this.runtime.settings.workDir, '../_reports/configs', `${reportId}.json`);
         ...
         */
        advancedReportsApp.post('/api/advanced-reports/generate-with-data/:id', this.secureFnc, async (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
         ...
         */
        advancedReportsApp.get('/api/advanced-reports/configs/:id', this.secureFnc, (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
         ...
         */
        advancedReportsApp.get('/api/advanced-reports/templates', this.secureFnc, (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
                ...
                try {
                    const templatesDir = path.join(this.runtime.settings.workDir, '../_reports/templates');
         ...
         */
        advancedReportsApp.get('/api/advanced-reports/templates/:name/template.json', this.secureFnc, (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
         ...
         */
        advancedReportsApp.get('/api/advanced-reports/templates/:name/thumbnail.png', this.secureFnc, (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
         ...
         * POST save template
         */
        advancedReportsApp.post('/api/advanced-reports/templates', this.secureFnc, (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
         ...
         * PUT update template
         */
        advancedReportsApp.put('/api/advanced-reports/templates/:name', this.secureFnc, (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
         ...
         * DELETE template
         */
        advancedReportsApp.delete('/api/advanced-reports/templates/:name', this.secureFnc, (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
         ...
         */
        advancedReportsApp.post('/api/advanced-reports/configs', this.secureFnc, (req, res) => {
            var groups = this.checkGroupsFnc(req);
            if (res.statusCode === 403) {
    ...
    async onTagChanged(tagEvent) {
        try {
            ...
            const triggeredReports = allReports.filter((report) => {
                // Check old format (triggerTags array)
                if (report.triggerTags && Array.isArray(report.triggerTags)) {
    ...
    async generatePdfFromTemplate(report, sendEmail = true) {
        try {
            ...
            const defaultDir = path.join(this.runtime.settings.workDir, '../_reports/generated');
            const settings = report.pdfmeData ? report.pdfmeData.settings : (report.pdfmeSettings || {});
            const savePath = settings.savePath || null;
            const reportsDir = savePath ? (path.isAbsolute(savePath) ? savePath : path.join(this.runtime.settings.workDir, '../_reports', savePath)) : defaultDir;
            const pdf = await (0, generator_1.generate)({
            ...
            this.runtime.logger.error(`Error in generatePdfFromTemplate: ${error.message}`);
            throw error;
        }
    }
    async generatePdfFromTemplateWithData(report, reportData = {}, tableData = {}) {
        try {
            // Get the template from the report
            const template = report.pdfmeData ? report.pdfmeData.template : report.template;
            if (!template) {
            ...
            this.runtime.logger.error(`Error in generatePdfFromTemplateWithData: ${error.message}`);
            throw error;
        }
    }
    ...
    isRisingEdge(previousValue, newValue) {
    ...
    getCronFromSchedule(scheduling) {
        const now = new Date();
        switch (scheduling) {
            case 'minute':
                // Every minute
